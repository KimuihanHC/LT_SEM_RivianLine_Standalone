...
LONG CSerialEx::StartListener (void)
{
	...
	if (m_hThread == 0)
	{
		...
		m_hThread = HANDLE(_beginthreadex(NULL, 0, ThreadProc, LPVOID(this), 0, &dwThreadId));
		if (m_hThread == 0)
		{
			// Display a warning
...
LONG CSerialEx::StopListener (DWORD dwTimeout)
{
	...
	if (m_hThread)
	{
		...
		WaitForSingleObject(m_hThread, STATUS_TIMEOUT);	
...
UINT WINAPI CSerialEx::ThreadProc (LPVOID lpArg)
{
...
DWORD CSerialEx::ThreadProc (void)
{
	...
	// Bye bye
	OutputDebugString(_T("쓰레드 종료 : CSerialEx::ThreadProc (void)\n"));
	return 0;
}