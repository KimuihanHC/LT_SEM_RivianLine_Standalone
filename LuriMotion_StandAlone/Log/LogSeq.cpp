...
BOOL CLogSeq::OpenLogFile( CExString& strLogPathName )
{
	...
	if ( !m_File.Open(strLogPathName, CFile::modeRead|CFile::shareDenyWrite) )
	{
		...
		CString machineName;
		machineName.Format(_T("%s"), g_szInsptrSysType[GET_INSPECTOR]);
		CExString strComment;
		strComment.Format( _T("#################### Luritech Seq Log ####################\r\n")
						   _T("- Created by LogSeq\r\n")
						   _T("- Machine Name: %s\r\n")
						   _T("- Date: %s\r\n")
						   _T("- SW Ver : %s.%s \r\n")
						   _T("#########################################################\r\n")
						   , machineName, strTime, GetVersionInfo(_T("ProductVersion")), GetVersionInfo(_T("FileVersion")));
...
void CLogSeq::CsvOut(LPCTSTR lpszId, LPCTSTR lpszFormat, ...)
{
	EnterCriticalSection(&m_csFile);
	if (lpszFormat == NULL || !m_bStart)
	{
